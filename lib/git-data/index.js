...
const gitData = {
    ...
    get name() {
        return memory.name = memory.name || (ENV_PROJECT.reduce((result, name) => result || process.env[name], undefined) || '').split('/').pop();
    },
    get branch() {
        return memory.branch = memory.branch || ENV_BRANCH.reduce((result, name) => result || process.env[name], undefined) || gitData.info.branch;
    },
    get tag() {
        return memory.tag = memory.tag || gitData.info.tag;
    },
    get author() {
        return memory.author = memory.author || ENV_AUTHOR.reduce((result, name) => result || process.env[name], undefined) || gitData.info.author || 'Publishing bot :robot_face:';
    },
    get sha() {
