...
module.exports = (string, data) => ({
    plain: clearLinks(phrase(string, convertKeys(data))),
    toString: () => clearLinks(phrase(string, convertKeys(data))),
    console: clearLinks(phrase(string, convertKeys(data, (item => item.underline)))),
    md: phrase(string, convertKeys(data, (item => `*${item}*`))),
});

const clearLinks = (str) => str.replace(/(<.*\|)|>/g, '')
