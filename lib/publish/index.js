...
module.exports = async ({testing = false} = {}) => {
    ...
    try {
        ...
        if (!exists || !await packageExists.call(npm, `${name}@${version}`)) {
            result = await publish({author, npm, exists, name, version, tag, title, title_link, testing});
        }
        ...
        await webhookNotify(
            await formatSlackMessage(`Error in publishing flow: ${error}`, {title, title_link, status: 'error'})
        );
...
async function publish({author, npm, exists, name, version, tag, title, title_link, testing = true}) {
    const lines = [
    ...
    await webhookNotify(
        await formatSlackMessage(msg.md, {title, title_link, status: 'success'})
    );
...
async function release({npm, name, version, tag, title, title_link, testing = true}) {
    ...
    await webhookNotify(
        await formatSlackMessage(msg.md, {title, title_link})
    );
